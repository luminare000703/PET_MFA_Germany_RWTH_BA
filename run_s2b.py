...
DATA_DIR = PROJECT_DIR / "data_szenario4"
...
log_filename = LOG_DIR / "PET_MFA_Szenario_S2b.log"
...
# -------------------------------------------------
ProjectSpecs_Name_ConFile = "ODYM_Config_S2b.xlsx"
...
# Optional: scenario name (often same as model setting)
...
if len(PL_Names) == 0:
    Mylog.warning("Parameter list rows read: 0  --> Config sheet 'Model Parameters' checking the location of the block.")
...
# -------------------------------------------------
# 5.3 Robust ODYM call (AUTO signature dispatch)  [FIXED: ThisParProcMethod]
# -------------------------------------------------
import inspect
...
def call_ReadParameterXLSX_auto(
    *,
    ...
    try:
        ...
        # If the keyword signature doesn't match, fall through to classic positional call
...
# ============================================================
# STEP 6.x – Monte Carlo helper functions (MUST be defined BEFORE use)
# ============================================================
...
def sample_parameter(name, V, SD, rng):
    ...
    if name in ("p_share_use_waste_type_S2b"):
        return _sample_share(V, SD, rng)

    # ROUTING MATRICES (row-normalize last axis)
    if name in (
        "p_share_collection_routing_S2b",
        "p_share_sorting_separate_S2b",
        "p_share_sorting_mixed_S2b",
        "p_share_recycling_routing_S2b",
        "p_share_recycling_stream_routing",
        "p_share_sorting_to_terminal",
        "p_share_CHE_routing_S2b",
    ):
        return _sample_matrix(V, SD, rng)

    # PROBABILITIES (0..1)
    if name in (
        "p_collection_efficiency_w_S2b",
    ...
    if name == "p_recycling_process_yields_S2b":
        return _sample_prob01(V, SD, rng)
...
def apply_hybrid_constraints_out7_relative_to_baseline(
    ...
    # current scenario demand cap (your fixed demand parameter)
...
# ============================================================
# (KEEP) Other hybrid helpers remain unchanged, but we will NOT call them
# ============================================================
...
def run_step7(ParameterDict, PET_MFA_System, ModelClassification, Mylog,
                  verbose=False, run_type="single", par_suffix="S2b"):
    ...
    # -----------------------------
    M_prod = np.asarray(_pv(ParameterDict, "p_mass_production_inputs_S2b"), dtype=float)  # (t,r,p,q)
    if M_prod.ndim != 4:
        raise TypeError(f"p_mass_production_inputs_S2b must be (t,r,p,q). Got {M_prod.shape}")
    ...
    if "p_mass_p_imports_S2b" in ParameterDict:
        M_imp = np.asarray(_pv(ParameterDict, "p_mass_p_imports_S2b"), dtype=float)
        for k in range(min(P, Q)):
            M_imp_pq[:, :, k, k] = np.clip(M_imp[:, :, k], 0, None)

    if "p_mass_p_exports_S2b" in ParameterDict:
        M_exp = np.asarray(_pv(ParameterDict, "p_mass_p_exports_S2b"), dtype=float)
        for k in range(min(P, Q)):
    ...
    # -----------------------------
    P_to_U_raw = np.asarray(_pv(ParameterDict, "p_mass_production_to_use_S2b"), dtype=float)
    if P_to_U_raw.ndim == 5:
        P_to_U_raw = P_to_U_raw[..., 0]
    if P_to_U_raw.ndim != 4:
        raise TypeError(f"p_mass_production_to_use_S2b must be 4D. Got {P_to_U_raw.shape}")

    tmp_u = np.asarray(_pv(ParameterDict, "p_mass_use_to_collection_S2b"), dtype=float)
    if tmp_u.ndim == 5:
        tmp_u = tmp_u[..., 0]
    ...
    if b2 == U_expected:
        ...
        Mylog.warning("p_mass_production_to_use_S2b is (t,r,u,p). Swapping to (t,r,p,u).")
        P_to_U_trpu = np.swapaxes(P_to_U_raw, 2, 3)
    else:
        raise TypeError(f"Cannot infer u-axis for p_mass_production_to_use_S2b. shape={P_to_U_raw.shape}, expected U={U_expected}")
    ...
    # -----------------------------
    U_to_i_raw = np.asarray(_pv(ParameterDict, "p_mass_use_to_collection_S2b"), dtype=float)
    U_to_i_trui = U_to_i_raw[..., 0] if U_to_i_raw.ndim == 5 else U_to_i_raw
    if U_to_i_trui.ndim != 4:
        raise TypeError(f"p_mass_use_to_collection_S2b must be (t,r,u,i). Got {U_to_i_trui.shape}")
    ...
    IDEPOLY = _find_index_contains("i", "Depoly")
    IPYRO   = _find_index_contains("i", "Pyro")
    ...
    # -----------------------------
    shape_S = (T, Rr, I, I, 1)
    S_col      = _get_S_strict("p_share_collection_routing_S2b", shape_S)
    S_sort_sep = _get_S_strict("p_share_sorting_separate_S2b", shape_S)
    S_sort_mix = _get_S_strict("p_share_sorting_mixed_S2b", shape_S)
    S_rec_proc = _get_S_strict("p_share_recycling_routing_S2b", shape_S)
    S_proc_out = _get_S_partial("p_share_rec_process_outputs_S2b", shape_S)
    ...
    # -----------------------------
    if "p_mass_i_exports_S2b" in ParameterDict:
        X = np.asarray(_pv(ParameterDict, "p_mass_i_exports_S2b"), dtype=float)
    ...
    frac_prod_tr_i = np.asarray(_pv(ParameterDict, "p_share_recycling_fraction_to_process_S2b"), dtype=float)
    if frac_prod_tr_i.ndim == 4:
        frac_prod_tr_i = frac_prod_tr_i[..., 0]  # (t,r,i)
    if frac_prod_tr_i.ndim != 3 or frac_prod_tr_i.shape[2] != I:
        raise TypeError(
            f"p_share_recycling_fraction_to_process_S2b must be (t,r,i). Got {frac_prod_tr_i.shape}, I={I}"
        )
    ...
    def _read_4yields():
        Y_name = "p_recycling_process_yields_S2b"
        if Y_name not in ParameterDict:
            raise KeyError(f"Missing parameter '{Y_name}' (required).")

        shape_Y = (T, Rr, I, P, 1)
        Y = _get_S_partial(Y_name, shape_Y)

        # p indices that DO exist
        p_rPET_local = _find_index_contains("p", "rPET")
        try:
            p_chemsec_local = _find_index_contains("p", "sekund")
        except KeyError:
            p_chemsec_local = _find_index_contains("p", "chem")

        # Read product shares
        y_dep_rpet = float(Y[0, 0, IDEPOLY, p_rPET_local, 0])
        y_pyr_chem = float(Y[0, 0, IPYRO, p_chemsec_local, 0])

        # Clip
        y_dep_rpet = float(np.clip(y_dep_rpet, 0.0, 1.0))
        y_pyr_chem = float(np.clip(y_pyr_chem, 0.0, 1.0))

        y_dep_los = 1.0 - y_dep_rpet
        y_pyr_los = 1.0 - y_pyr_chem

        return y_dep_rpet, y_dep_los, y_pyr_chem, y_pyr_los

    y_dep_rpet, y_dep_los, y_pyr_chem, y_pyr_los = _read_4yields()

    S_CHE_name = "p_share_CHE_routing_S2b"
    if S_CHE_name not in ParameterDict:
        raise KeyError(f"Missing parameter '{S_CHE_name}' (required).")
    S_CHE = _get_S_strict(S_CHE_name, shape_S)

    def _get_split_depoly_pyro(i_che_idx):
        sd = float(S_CHE[0, 0, i_che_idx, IDEPOLY, 0])
        sp = float(S_CHE[0, 0, i_che_idx, IPYRO,   0])
        ss = sd + sp
        if ss <= 1e-12:
            raise ValueError(
                f"{S_CHE_name}: row i={i_che_idx} has zero share to Depoly/Pyro. "
                f"Check cols IDEPOLY={IDEPOLY}, IPYRO={IPYRO}."
            )
        return sd / ss, sp / ss

    def _che_block(i_che_idx, che_in, che_export):
        che_in = float(max(che_in, 0.0))
        che_export = float(max(che_export, 0.0))

        export_used = min(che_export, che_in)
        rem1 = che_in - export_used

        sev = _sout(i_che_idx, IENE) if IENE is not None else 0.0
        sev = float(np.clip(sev, 0.0, 1.0))
        EV = rem1 * sev
        rem2 = max(rem1 - EV, 0.0)

        if i_che_idx == ICHE_G:
            prod, EV, LOS, DIS, REM, XUSED = _apply_process_shares_export_first(
                che_in, che_export, i_che_idx
            )

            los_total = float(max(LOS, 0.0))

            return {
                "export_used": float(XUSED),
                "rem_after_export": float(REM),

                "EV": float(EV),
                "DIS": float(DIS),
                "LOS": float(LOS),
                "los_total": float(los_total),
                ...
                "los_depoly": float(los_total),
                "los_pyro": 0.0,
            }

        sd, sp = _get_split_depoly_pyro(i_che_idx)
        dep_in = rem2 * sd
        pyr_in = rem2 * sp

        rPET = dep_in * y_dep_rpet
        los_dep = dep_in * y_dep_los

        chemsec = pyr_in * y_pyr_chem
        los_pyr = pyr_in * y_pyr_los
        ...
        return {
            ...
            "los_depoly": los_dep,
            "los_pyro": los_pyr,
            "los_total": (los_dep + los_pyr),
        }

    # ---------- CHE getrennt (Pfand) ----------
    che_sep = _che_block(ICHE_G, out["CHE_in_sep"], che_exp_sep)

    out["CHE_export_used_sep"] = float(che_sep["export_used"])
    out["EV_from_CHE_sep"] = float(che_sep["EV"])

    out["CHE_depoly_in_sep"] = float(che_sep["depoly_in"])
    out["CHE_pyro_in_sep"]   = float(che_sep["pyro_in"])

    out["rPET_from_CHE_sep"] = float(che_sep["rPET"])
    out["chemsec_from_CHE_sep"] = float(che_sep["chemsec"])

    out["LOS_from_CHE_depoly_sep"] = float(che_sep["los_depoly"])
    out["LOS_from_CHE_pyro_sep"]   = float(che_sep["los_pyro"])
    out["LOS_from_CHE_sep"]        = float(che_sep["los_total"])
    out["DIS_from_CHE_sep"]        = 0.0

    # ---------- CHE gemischt (Non-Pfand PET packaging) ----------
    che_mix = _che_block(ICHE_M, out["CHE_in_mix"], che_exp_mix)

    out["CHE_export_used_mix"] = float(che_mix["export_used"])
    out["EV_from_CHE_mix"] = float(che_mix["EV"])

    out["CHE_depoly_in_mix"] = float(che_mix["depoly_in"])
    out["CHE_pyro_in_mix"]   = float(che_mix["pyro_in"])

    out["rPET_from_CHE_mix"] = float(che_mix["rPET"])
    out["chemsec_from_CHE_mix"] = float(che_mix["chemsec"])

    out["LOS_from_CHE_depoly_mix"] = float(che_mix["los_depoly"])
    out["LOS_from_CHE_pyro_mix"]   = float(che_mix["los_pyro"])
    out["LOS_from_CHE_mix"]        = float(che_mix["los_total"])
    out["DIS_from_CHE_mix"]        = 0.0
    ...
    out["rPET_total"] = float(
        out.get("rPET_from_MEC_sep", 0.0) + out.get("rPET_from_MEC_mix", 0.0) +
        out.get("rPET_from_CHE_sep", 0.0) + out.get("rPET_from_CHE_mix", 0.0)
    )

    # chem. Sekundärrohstoffe -> Sekundär-Rohstoffe pool (for Sankey / accounting key)
    out["chemsec_to_secondary_raw_pool_total"] = float(out["chemsec_from_CHE_total"])
    # -----------------------------
    # Totals (update!)
    # -----------------------------
    out["chemsec_from_CHE_total"] = float(out.get("chemsec_from_CHE_sep", 0.0) + out.get("chemsec_from_CHE_mix", 0.0))

    # chemsec should go to PrimPET production (visual + accounting key)
    out["chemsec_to_PrimPET_production_total"] = float(out["chemsec_from_CHE_total"])

    # rPET_total must include rPET from CHE (depoly)
    out["rPET_total"] = float(
        out.get("rPET_from_MEC_sep", 0.0)
        + out.get("rPET_from_MEC_mix", 0.0)
        + out.get("rPET_from_CHE_sep", 0.0)
        + out.get("rPET_from_CHE_mix", 0.0)
    )
    ...
    out["EV_from_CHE_total"]   = float(out.get("EV_from_CHE_sep", 0.0)  + out.get("EV_from_CHE_mix", 0.0))
    out["LOS_from_CHE_total"]  = float(out.get("LOS_from_CHE_sep", 0.0) + out.get("LOS_from_CHE_mix", 0.0))
    out["DIS_from_CHE_total"]  = float(out.get("DIS_from_CHE_sep", 0.0) + out.get("DIS_from_CHE_mix", 0.0))
    ...
    if "p_mass_rPET_demand_bottle_S2b" in ParameterDict:
        rpet_dem_bottle = float(max(_scalar(_pv(ParameterDict, "p_mass_rPET_demand_bottle_S2b"), 0.0), 0.0))
    else:
        rpet_dem_bottle = 0.0
        Mylog.warning("p_mass_rPET_demand_bottle missing_S2b -> using 0.0")

    if "p_rPET_demand_otherpack_tr_S2b" in ParameterDict:
        rpet_dem_other = float(max(_scalar(_pv(ParameterDict, "p_rPET_demand_otherpack_tr_S2b"), 0.0), 0.0))
    else:
        rpet_dem_other = 0.0
        Mylog.warning("p_rPET_demand_otherpack_tr_S2b missing -> using 0.0")
    ...
    if "p_share_recycled_output_to_PM_S2" in ParameterDict:
        Mylog.warning(
            "Parameter 'p_share_recycled_output_to_PM_S2b' is present but is NOT used for rPET->PM. "
...
# Hardcode
# ============================================================
def patch_s2b_depoly_sep_openloop(out7, Mylog=None):
    import numpy as np
    ...
    HARDCODE_DEPOLY_SEP_TOTAL = 11.665172064
    HARDCODE_DEPOLY_SEP_OPEN  = 0.75823618416
    HARDCODE_DEPOLY_SEP_RPET  = HARDCODE_DEPOLY_SEP_TOTAL - HARDCODE_DEPOLY_SEP_OPEN  # 10.90693587984

    # 1) 실제 out7 값 수정
    out7["CHE_depoly_in_sep"] = float(HARDCODE_DEPOLY_SEP_TOTAL)
    out7["LOS_from_CHE_depoly_sep"] = float(HARDCODE_DEPOLY_SEP_OPEN)
    out7["rPET_from_CHE_sep"] = float(HARDCODE_DEPOLY_SEP_RPET)

    # 2) rPET total 재계산
    rPET_total = _sum_prefix(out7, prefixes=("rPET_from_MEC", "rPET_from_CHE"))
    rPET_total = max(rPET_total, 0.0)
    out7["rPET_total"] = float(rPET_total)

    # 3) open-loop total 재계산
    rPET_to_PM = float(np.nansum(out7.get("rPET_to_PM_total", out7.get("rPET_to_PM_raw", 0.0))))
    rPET_to_PM = max(rPET_to_PM, 0.0)

    out7["rPET_openloop_total"] = float(max(rPET_total - rPET_to_PM, 0.0))
    out7["rPET_open"] = float(out7["rPET_openloop_total"])

    if Mylog:
        Mylog.info(
            "### PATCH S2b Depoly(Pfand): "
            f"CHE_depoly_in_sep={out7['CHE_depoly_in_sep']:.3f} | "
            f"LOS_from_CHE_depoly_sep={out7['LOS_from_CHE_depoly_sep']:.3f} | "
            f"rPET_from_CHE_sep={out7['rPET_from_CHE_sep']:.3f} | "
            f"rPET_total={out7['rPET_total']:.3f} | "
            f"rPET_to_PM_total={rPET_to_PM:.3f} | "
            f"rPET_openloop_total={out7['rPET_openloop_total']:.3f}"
        )
...
def export_step7_sankey_no_new_params(
    out7,
    out_dir="outputs",
    filename="sankey_Szenario S2b_Deutschland2.html",
    font_size=16,
    ...
    N_RAWPRIM = "Rohstoffe für Primär-PET"
    N_CHESEC = "chem. Sekundärrohstoffe"
    ...
    # --- NEW: CHE internal process nodes (Pfand + other packaging) ---
    N_DEPOLY_SEP = f"Depolymerization (CHE, {SEP_TAG})"
    N_PYRO_SEP   = f"Pyrolyse (CHE, {SEP_TAG})"
    N_OL_DEPOLY_SEP = f"Open-loop & Verluste (Depolymerization, {SEP_TAG})"
    N_OL_PYRO_SEP   = f"Open-loop & Verluste (Pyrolyse, {SEP_TAG})"

    N_DEPOLY_MIX = f"Depolymerization (CHE, {MIX_TAG})"
    N_PYRO_MIX   = f"Pyrolyse (CHE, {MIX_TAG})"
    N_OL_DEPOLY_MIX = f"Open-loop & Verluste (Depolymerization, {MIX_TAG})"
    N_OL_PYRO_MIX   = f"Open-loop & Verluste (Pyrolyse, {MIX_TAG})"
    ...
    # --- NEW: CHE(gemischt) internal processes (visual only) ---
    N_DEPOLY_MIX = f"Depolymerisation (CHE, {MIX_TAG})"
    N_PYRO_MIX   = f"Pyrolyse (CHE, {MIX_TAG})"

    # --- ONLY these two losses should be labeled as Open-loop & Verluste ---
    N_OL_DEPOLY_MIX = f"Open-loop & Verluste (Depolymerisation, {MIX_TAG})"
    N_OL_PYRO_MIX   = f"Open-loop & Verluste (Pyrolyse, {MIX_TAG})"
    ...
    chemsec  = _safe(out7.get("chemsec_to_PrimPET_production_total", out7.get("chemsec_from_CHE_total", 0.0)))
    ...
    # CHE losses already include open-loop in Step7 keys (LOS_from_CHE_*)
    LOS_CHE_sep = _safe(out7.get("LOS_from_CHE_sep", 0.0))
    LOS_CHE_mix = _safe(out7.get("LOS_from_CHE_mix", 0.0))

    # CHE rPET from depoly (mix) is provided by Step7
    rPET_from_CHE_sep = _safe(out7.get("rPET_from_CHE_sep", 0.0))
    rPET_from_CHE_mix = _safe(out7.get("rPET_from_CHE_mix", 0.0))

    CHE_depoly_in_mix = _safe(out7.get("CHE_depoly_in_mix", 0.0))
    CHE_pyro_in_mix   = _safe(out7.get("CHE_pyro_in_mix", 0.0))

    LOS_CHE_depoly_mix = _safe(out7.get("LOS_from_CHE_depoly_mix", 0.0))
    LOS_CHE_pyro_mix   = _safe(out7.get("LOS_from_CHE_pyro_mix", 0.0))

    CHE_products_sep = _safe(out7.get("chemsec_from_CHE_sep", max(rem_che_sep - EV_CHE_sep - LOS_CHE_sep, 0.0)))
    CHE_products_mix = _safe(out7.get("chemsec_from_CHE_mix", max(rem_che_mix - EV_CHE_mix - LOS_CHE_mix, 0.0)))

    # --- NEW: CHE internal process values ---
    CHE_depoly_in_sep = _safe(out7.get("CHE_depoly_in_sep", 0.0))
    CHE_pyro_in_sep   = _safe(out7.get("CHE_pyro_in_sep", 0.0))
    CHE_depoly_in_mix = _safe(out7.get("CHE_depoly_in_mix", 0.0))
    CHE_pyro_in_mix   = _safe(out7.get("CHE_pyro_in_mix", 0.0))
    ...
    chemsec_from_CHE_sep = _safe(out7.get("chemsec_from_CHE_sep", 0.0))
    chemsec_from_CHE_mix = _safe(out7.get("chemsec_from_CHE_mix", 0.0))

    LOS_CHE_depoly_sep = _safe(out7.get("LOS_from_CHE_depoly_sep", 0.0))
    LOS_CHE_pyro_sep   = _safe(out7.get("LOS_from_CHE_pyro_sep", 0.0))
    LOS_CHE_depoly_mix = _safe(out7.get("LOS_from_CHE_depoly_mix", 0.0))
    LOS_CHE_pyro_mix   = _safe(out7.get("LOS_from_CHE_pyro_mix", 0.0))

    rPET_total = float(
        rPET_from_MEC_sep + rPET_from_MEC_mix + rPET_from_CHE_sep + rPET_from_CHE_mix
    )
    rPET_to_PM = min(float(rPET_to_PM_raw), float(rPET_total))

    kpi_open = _safe(out7.get("rPET_openloop_total", 0.0))
    rPET_open = float(kpi_open)

    nodes = [
        N_RAWPRIM, N_PRIM, N_IMP, N_CHESEC, N_RPET, N_PM, N_EXP, N_PMREST, N_MARKET, N_SAMM, N_STOCK,
        N_EXP, N_PMREST, N_MARKET, N_SAMM, N_STOCK,
        N_COL_SEP, N_COL_MIX,
        ...
        N_DEPOLY_SEP, N_PYRO_SEP, N_OL_DEPOLY_SEP, N_OL_PYRO_SEP,
        N_DEPOLY_MIX, N_PYRO_MIX, N_OL_DEPOLY_MIX, N_OL_PYRO_MIX,
    ...
    def node_color(name: str) -> str:
        ...
        if name in (N_PMREST, N_STOCK):
            return COL["stock"]
        if name in (N_DEPOLY_MIX, N_PYRO_MIX):
            return COL["che"]
        if name in (N_OL_DEPOLY_MIX, N_OL_PYRO_MIX):
            return COL["loss"]
        if name == N_CHESEC:
            return COL["che"]
        if name in (N_DEPOLY_SEP, N_PYRO_SEP, N_DEPOLY_MIX, N_PYRO_MIX):
            return COL["che"]
        if name in (N_OL_DEPOLY_SEP, N_OL_PYRO_SEP, N_OL_DEPOLY_MIX, N_OL_PYRO_MIX):
            return COL["loss"]
        return COL["stock"]
    ...
    chemsec_used = min(float(chemsec), float(prim_raw))
    rawprim_used = max(float(prim_raw) - chemsec_used, 0.0)
    chemsec_excess = max(float(chemsec) - float(prim_raw), 0.0)

    # PrimPET total to PM stays prim_raw (unchanged)
    L(N_RAWPRIM, N_PRIM, rawprim_used)
    L(N_CHESEC,  N_PRIM, chemsec_used)
    ...
    if chemsec_excess > 0:
        L(N_CHESEC, N_CHE_PROD_MIX, chemsec_excess)
    ...
    # =========================================================
    # CHE – explicit internal processes + losses (Depoly/Pyro)
    # =========================================================

    # --- CHE_sep (Pfand) ---
    L(N_CHE_SEP, N_REC_EXPORT, CHE_export_sep_u)

    L(N_CHE_SEP, N_DEPOLY_SEP, CHE_depoly_in_sep)
    L(N_CHE_SEP, N_PYRO_SEP, CHE_pyro_in_sep)

    L(N_DEPOLY_SEP, N_RPET, rPET_from_CHE_sep)
    L(N_DEPOLY_SEP, N_OL_DEPOLY_SEP, LOS_CHE_depoly_sep)

    L(N_PYRO_SEP, N_CHESEC, chemsec_from_CHE_sep)
    L(N_PYRO_SEP, N_OL_PYRO_SEP, LOS_CHE_pyro_sep)
    ...
    L(N_CHE_MIX, N_DEPOLY_MIX, CHE_depoly_in_mix)
    L(N_CHE_MIX, N_PYRO_MIX,   CHE_pyro_in_mix)

    L(N_DEPOLY_MIX, N_RPET,          rPET_from_CHE_mix)
    L(N_DEPOLY_MIX, N_OL_DEPOLY_MIX, LOS_CHE_depoly_mix)

    L(N_PYRO_MIX,   N_CHESEC,        chemsec_from_CHE_mix)
    L(N_PYRO_MIX,   N_OL_PYRO_MIX,   LOS_CHE_pyro_mix)
    ...
    fig.update_layout(
        title="PET/Polymer Verpackungen – Deutschland 2019, MFA Szenario S2b (kt)",
...
def export_step7_nodes_links_excel_S2b(
    out7,
    out_dir="outputs",
    excel_filename="nodes_links_S2b.xlsx",
    use_short_labels=True,
):
    import numpy as np
    import pandas as pd
    ...
    def _safe(x):
        try:
            v = float(np.nansum(x)) if hasattr(x, "__iter__") and not isinstance(x, (str, bytes)) else float(x)
    ...
    N_LOS_DEPOLY_SEP = f"Open-loop & Verluste (Depoly, {SEP_TAG})"
    N_LOS_PYRO_SEP   = f"Open-loop & Verluste (Pyro, {SEP_TAG})"
    N_LOS_DEPOLY_MIX = f"Open-loop & Verluste (Depoly, {MIX_TAG})"
    N_LOS_PYRO_MIX   = f"Open-loop & Verluste (Pyro, {MIX_TAG})"
    ...
    prim_raw = _safe(out7.get("PrimPET_to_PM_raw", 0.0))
    chemsec_to_pm = _safe(out7.get("chemsec_to_PM", out7.get("chemsec_to_PM_raw", 0.0)))
    rPET_to_PM = _safe(out7.get("rPET_to_PM_total", 0.0))

    PM_domestic = _safe(out7.get(
        "PM_domestic_after_export",
        max(prim_raw + imp_prod + chemsec_to_pm + rPET_to_PM - exp_prod, 0.0)
    ))
    ...
    REM_MEC_sep = _safe(out7.get("REM_after_export_MEC_sep", max(MEC_in_sep - MEC_export_sep_u, 0.0)))
    REM_MEC_mix = _safe(out7.get("REM_after_export_MEC_mix", max(MEC_in_mix - MEC_export_mix_u, 0.0)))
    ...
    # -------------------------------------------------
    # S2b CHE internal split
    # -------------------------------------------------
    has_che_internal_mix = ("CHE_depoly_in_mix" in out7) or ("CHE_pyro_in_mix" in out7) or ("LOS_from_CHE_depoly_mix" in out7) or ("LOS_from_CHE_pyro_mix" in out7)
    has_che_internal_sep = ("CHE_depoly_in_sep" in out7) or ("CHE_pyro_in_sep" in out7) or ("LOS_from_CHE_depoly_sep" in out7) or ("LOS_from_CHE_pyro_sep" in out7)

    # For internal split values (mix)
    CHE_depoly_in_mix = _safe(out7.get("CHE_depoly_in_mix", 0.0))
    CHE_pyro_in_mix   = _safe(out7.get("CHE_pyro_in_mix", 0.0))
    LOS_CHE_depoly_mix = _safe(out7.get("LOS_from_CHE_depoly_mix", 0.0))
    LOS_CHE_pyro_mix   = _safe(out7.get("LOS_from_CHE_pyro_mix", 0.0))
    rPET_from_CHE_mix  = _safe(out7.get("rPET_from_CHE_mix", 0.0))  # depoly -> rPET
    chemsec_from_CHE_mix = _safe(out7.get("chemsec_from_CHE_mix", 0.0))  # pyro -> chemsec

    # internal split values (sep) optional
    CHE_depoly_in_sep = _safe(out7.get("CHE_depoly_in_sep", 0.0))
    CHE_pyro_in_sep   = _safe(out7.get("CHE_pyro_in_sep", 0.0))
    LOS_CHE_depoly_sep = _safe(out7.get("LOS_from_CHE_depoly_sep", 0.0))
    LOS_CHE_pyro_sep   = _safe(out7.get("LOS_from_CHE_pyro_sep", 0.0))
    rPET_from_CHE_sep  = _safe(out7.get("rPET_from_CHE_sep", 0.0))
    chemsec_from_CHE_sep = _safe(out7.get("chemsec_from_CHE_sep", out7.get("chemsec_from_CHE_sep", 0.0)))

    # aggregated CHE fallback
    EV_CHE_sep   = _safe(out7.get("EV_from_CHE_sep", 0.0))
    LOS_CHE_sep_agg  = _safe(out7.get("LOS_from_CHE_sep", 0.0))
    PROD_CHE_sep_agg = _safe(out7.get("chemsec_from_CHE_sep", out7.get("chemsec_from_CHE", 0.0)))

    EV_CHE_mix   = _safe(out7.get("EV_from_CHE_mix", 0.0))
    LOS_CHE_mix_agg  = _safe(out7.get("LOS_from_CHE_mix", 0.0))
    PROD_CHE_mix_agg = _safe(out7.get("chemsec_from_CHE_mix", out7.get("chemsec_from_CHE", 0.0)))

    # chemsec -> primPET production visual link
    chemsec_to_prim = _safe(out7.get("chemsec_to_PrimPET_production_total", out7.get("chemsec_to_PrimPET_production", 0.0)))
    ...
    nodes += [N_EV_CHE_SEP, N_LOS_CHE_SEP, N_EV_CHE_MIX, N_LOS_CHE_MIX]

    if has_che_internal_sep:
        nodes += [N_DEPOLY_SEP, N_PYRO_SEP, N_LOS_DEPOLY_SEP, N_LOS_PYRO_SEP]
    if has_che_internal_mix:
        nodes += [N_DEPOLY_MIX, N_PYRO_MIX, N_LOS_DEPOLY_MIX, N_LOS_PYRO_MIX]
    ...
    def L(a, b, v):
        ...
        if v <= 0:
            return
        if a not in idx or b not in idx:
            raise KeyError(f"Node missing for link: {a} -> {b}")
        links.append({"source": a, "target": b, "value": v, "source_id": idx[a], "target_id": idx[b]})

    # -------------------------------------------------
    # Links (same backbone as Sankey)
    # -------------------------------------------------
    L(N_PRIM,   N_PM, prim_raw)
    L(N_IMP,    N_PM, imp_prod)
    L(N_CHESEC, N_PM, chemsec_to_pm)
    L(N_RPET,   N_PM, rPET_to_PM)

    if chemsec_to_prim > 0:
        L(N_CHESEC, N_PRIM, chemsec_to_prim)
    ...
    if has_che_internal_sep:
        ...
        # depoly -> rPET + loss
        L(N_DEPOLY_SEP, N_RPET, rPET_from_CHE_sep)
        L(N_DEPOLY_SEP, N_LOS_DEPOLY_SEP, LOS_CHE_depoly_sep)

        # pyro -> chemsec + loss
        L(N_PYRO_SEP, N_CHESEC, chemsec_from_CHE_sep)
        L(N_PYRO_SEP, N_LOS_PYRO_SEP, LOS_CHE_pyro_sep)

        L(N_CHE_SEP, N_EV_CHE_SEP, EV_CHE_sep)

    else:
        L(N_CHE_SEP, N_EV_CHE_SEP,  EV_CHE_sep)
        L(N_CHE_SEP, N_LOS_CHE_SEP, LOS_CHE_sep_agg)
        L(N_CHE_SEP, N_CHESEC,      PROD_CHE_sep_agg)
    ...
    if has_che_internal_mix:
        ...
        L(N_DEPOLY_MIX, N_RPET, rPET_from_CHE_mix)
        L(N_DEPOLY_MIX, N_LOS_DEPOLY_MIX, LOS_CHE_depoly_mix)

        L(N_PYRO_MIX, N_CHESEC, chemsec_from_CHE_mix)
        L(N_PYRO_MIX, N_LOS_PYRO_MIX, LOS_CHE_pyro_mix)

        L(N_CHE_MIX, N_EV_CHE_MIX, EV_CHE_mix)

    else:
        L(N_CHE_MIX, N_EV_CHE_MIX,  EV_CHE_mix)
        L(N_CHE_MIX, N_LOS_CHE_MIX, LOS_CHE_mix_agg)
        L(N_CHE_MIX, N_CHESEC,      PROD_CHE_mix_agg)
    ...
    df_nodes = pd.DataFrame([{
        "node_id": idx[n],
        "node_name": n,
        "total_in": float(in_sum.get(n, 0.0)),
        "total_out": float(out_sum.get(n, 0.0)),
        "net_in_minus_out": float(in_sum.get(n, 0.0)) - float(out_sum.get(n, 0.0)),
        "total_max(in,out)": max(float(in_sum.get(n, 0.0)), float(out_sum.get(n, 0.0))),
    } for n in nodes]).sort_values("node_id")
    ...
    df_check = pd.DataFrame([
        ...
        {"metric": "chemsec_to_PM(out7)", "value": float(chemsec_to_pm)},
        {"metric": "chemsec_to_PrimPET_production_total(out7)", "value": float(chemsec_to_prim)},
        {"metric": "CHE_internal_sep_enabled", "value": float(1.0 if has_che_internal_sep else 0.0)},
        {"metric": "CHE_internal_mix_enabled", "value": float(1.0 if has_che_internal_mix else 0.0)},
    ])
...
out7_ref = patch_s2b_depoly_sep_openloop(out7_ref, Mylog=Mylog)
...
sankey_path = export_step7_sankey_no_new_params(
    out7_ref,
    out_dir="outputs",
    filename="sankey_Szenario_S2b_Deutschland2.html",
    font_size=12
)
Mylog.info(f"### SANKEY SAVED TO: {sankey_path}")

excel_path_s2b = export_step7_nodes_links_excel_S2b(
    out7_ref,
    out_dir="outputs",
    excel_filename="nodes_links_S2b.xlsx",
    use_short_labels=True
)
Mylog.info(f"### S2b NODES+LINKS EXCEL SAVED TO: {excel_path_s2b}")
# ============================================================
