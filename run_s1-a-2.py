...
DATA_DIR = PROJECT_DIR / "data_szenario2"
...
log_filename = LOG_DIR / "PET_MFA_Szenario_S1a2.log"
...
# -------------------------------------------------
ProjectSpecs_Name_ConFile = "ODYM_Config_S1a2.xlsx"
...
# ============================================
# STEP 4 – Build ModelClassification from Index Table
# ============================================
...
def sample_parameter(name, V, SD, rng):
    ...
    if name in ("p_share_use_waste_type_S1a2"):
        return _sample_share(V, SD, rng)

    # ROUTING MATRICES (row-normalize last axis)
    if name in (
        "p_share_collection_routing_S1a2",
        "p_share_sorting_separate_S1a2",
        "p_share_sorting_mixed_S1a2",
        "p_share_recycling_routing_S1a2",
    ...
    # PROBABILITIES (0..1)
    if name in (
        "p_collection_efficiency_w_S1a2",
    ...
    if name == "p_recycling_process_yields_S1a2":
        return _sample_prob01(V, SD, rng)
...
def run_step7(ParameterDict, PET_MFA_System, ModelClassification, Mylog,
                  verbose=False, run_type="single", par_suffix="S1a2"):
    ...
    # -----------------------------
    M_prod = np.asarray(_pv(ParameterDict, "p_mass_production_inputs_S1a2"), dtype=float)  # (t,r,p,q)
    if M_prod.ndim != 4:
        raise TypeError(f"p_mass_production_inputs_S1a2 must be (t,r,p,q). Got {M_prod.shape}")
    ...
    if "p_mass_p_imports_S1a2" in ParameterDict:
        M_imp = np.asarray(_pv(ParameterDict, "p_mass_p_imports_S1a2"), dtype=float)
        for k in range(min(P, Q)):
            M_imp_pq[:, :, k, k] = np.clip(M_imp[:, :, k], 0, None)

    if "p_mass_p_exports_S1a2" in ParameterDict:
        M_exp = np.asarray(_pv(ParameterDict, "p_mass_p_exports_S1a2"), dtype=float)
        for k in range(min(P, Q)):
    ...
    # -----------------------------
    P_to_U_raw = np.asarray(_pv(ParameterDict, "p_mass_production_to_use_S1a2"), dtype=float)
    if P_to_U_raw.ndim == 5:
        P_to_U_raw = P_to_U_raw[..., 0]
    if P_to_U_raw.ndim != 4:
        raise TypeError(f"p_mass_production_to_use_S1a2 must be 4D. Got {P_to_U_raw.shape}")

    tmp_u = np.asarray(_pv(ParameterDict, "p_mass_use_to_collection_S1a2"), dtype=float)
    if tmp_u.ndim == 5:
        tmp_u = tmp_u[..., 0]
    ...
    if b2 == U_expected:
        ...
        Mylog.warning("p_mass_production_to_use_S1a2 is (t,r,u,p). Swapping to (t,r,p,u).")
        P_to_U_trpu = np.swapaxes(P_to_U_raw, 2, 3)
    else:
        raise TypeError(f"Cannot infer u-axis for p_mass_production_to_use_S1a2. shape={P_to_U_raw.shape}, expected U={U_expected}")
    ...
    # -----------------------------
    U_to_i_raw = np.asarray(_pv(ParameterDict, "p_mass_use_to_collection_S1a2"), dtype=float)
    U_to_i_trui = U_to_i_raw[..., 0] if U_to_i_raw.ndim == 5 else U_to_i_raw
    if U_to_i_trui.ndim != 4:
        raise TypeError(f"p_mass_use_to_collection_S1a2 must be (t,r,u,i). Got {U_to_i_trui.shape}")
    ...
    # -----------------------------
    shape_S = (T, Rr, I, I, 1)
    S_col      = _get_S_strict("p_share_collection_routing_S1a2", shape_S)
    S_sort_sep = _get_S_strict("p_share_sorting_separate_S1a2", shape_S)
    S_sort_mix = _get_S_strict("p_share_sorting_mixed_S1a2", shape_S)
    S_rec_proc = _get_S_strict("p_share_recycling_routing_S1a2", shape_S)
    S_proc_out = _get_S_partial("p_share_rec_process_outputs_S1a2", shape_S)
    ...
    # -----------------------------
    if "p_mass_i_exports_S1a2" in ParameterDict:
        X = np.asarray(_pv(ParameterDict, "p_mass_i_exports_S1a2"), dtype=float)
    ...
    frac_prod_tr_i = np.asarray(_pv(ParameterDict, "p_share_recycling_fraction_to_process_S1a2"), dtype=float)
    if frac_prod_tr_i.ndim == 4:
        frac_prod_tr_i = frac_prod_tr_i[..., 0]  # (t,r,i)
    if frac_prod_tr_i.ndim != 3 or frac_prod_tr_i.shape[2] != I:
        raise TypeError(
            f"p_share_recycling_fraction_to_process_S1a2 must be (t,r,i). Got {frac_prod_tr_i.shape}, I={I}"
        )
    ...
    if "p_mass_rPET_demand_bottle_S1a2" in ParameterDict:
        rpet_dem_bottle = float(max(_scalar(_pv(ParameterDict, "p_mass_rPET_demand_bottle_S1a2"), 0.0), 0.0))
    else:
        rpet_dem_bottle = 0.0
        Mylog.warning("p_mass_rPET_demand_bottle missing_S1a2 -> using 0.0")

    if "p_rPET_demand_otherpack_tr_S1a2" in ParameterDict:
        rpet_dem_other = float(max(_scalar(_pv(ParameterDict, "p_rPET_demand_otherpack_tr_S1a2"), 0.0), 0.0))
    else:
        rpet_dem_other = 0.0
        Mylog.warning("p_rPET_demand_otherpack_tr_S1a2 missing -> using 0.0")
    ...
    if "p_share_recycled_output_to_PM_S1a2" in ParameterDict:
        Mylog.warning(
            "Parameter 'p_share_recycled_output_to_PM_S1a2' is present but is NOT used for rPET->PM. "
...
def export_step7_sankey_no_new_params(
    out7,
    out_dir="outputs",
    filename="sankey_Szenario S1a2_2040_Deutschland.html",
    font_size=16,
    ...
    fig.update_layout(
        title="PET/Polymer Verpackungen – Deutschland 2040, MFA Szenario S1-a2 (kt)",
...
def export_step7_nodes_links_excel_S1a2(
    out7,
    out_dir="outputs",
    excel_filename="nodes_links_S1a2.xlsx",
    use_short_labels=True,
):
    import numpy as np
    import pandas as pd
    ...
    # ----------------------------
    # Labels (S1a2 = no CHE internal Depoly/Pyro)
    # ----------------------------
    if use_short_labels:
        SEP_TAG  = "Pfandflaschen"
        MIX_TAG  = "andere PET-Verp."
    ...
    # CHE outputs (S1a2 baseline-style keys)
...
sankey_path = export_step7_sankey_no_new_params(
    out7_ref,
    out_dir="outputs",
    filename="sankey_Szenario_S1a2_Deutschland2.html",
    font_size=12
)
Mylog.info(f"### SANKEY SAVED TO: {sankey_path}")

excel_path = export_step7_nodes_links_excel_S1a2(
    out7_ref,
    out_dir="outputs",
    excel_filename="nodes_links_S1a2.xlsx",
    use_short_labels=True
)
Mylog.info(f"### S1a2 NODES+LINKS EXCEL SAVED TO: {excel_path}")
