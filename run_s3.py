...
DATA_DIR = PROJECT_DIR / "data_szenario5"
...
log_filename = LOG_DIR / "PET_MFA_Szenario_S3.log"
...
# -------------------------------------------------
ProjectSpecs_Name_ConFile = "ODYM_Config_S3.xlsx"
...
def sample_parameter(name, V, SD, rng):
    ...
    if name in ("p_share_use_waste_type_S3"):
        return _sample_share(V, SD, rng)

    # ROUTING MATRICES (row-normalize last axis)
    if name in (
        "p_share_collection_routing_S3",
        "p_share_sorting_separate_S3",
        "p_share_sorting_mixed_S3",
        "p_share_recycling_routing_S3",
        "p_share_recycling_stream_routing",
        "p_share_sorting_to_terminal",
        "p_share_CHE_routing_S3",
    ):
        return _sample_matrix(V, SD, rng)

    # PROBABILITIES (0..1)
    if name in (
        "p_collection_efficiency_w_S3",
    ...
    if name == "p_recycling_process_yields_S3":
        return _sample_prob01(V, SD, rng)
...
def apply_hybrid_constraints_out7_relative_to_baseline(
    ...
    out7["rPET_total"] = float(rPET_supply)
...
# ============================================================
# (KEEP) Other hybrid helpers remain unchanged
# ============================================================
...
def run_step7(ParameterDict, PET_MFA_System, ModelClassification, Mylog,
                  verbose=False, run_type="single", par_suffix="S3"):
    ...
    # -----------------------------
    M_prod = np.asarray(_pv(ParameterDict, "p_mass_production_inputs_S3"), dtype=float)  # (t,r,p,q)
    if M_prod.ndim != 4:
        raise TypeError(f"p_mass_production_inputs_S3 must be (t,r,p,q). Got {M_prod.shape}")
    ...
    if "p_mass_p_imports_S3" in ParameterDict:
        M_imp = np.asarray(_pv(ParameterDict, "p_mass_p_imports_S3"), dtype=float)
        for k in range(min(P, Q)):
            M_imp_pq[:, :, k, k] = np.clip(M_imp[:, :, k], 0, None)

    if "p_mass_p_exports_S3" in ParameterDict:
        M_exp = np.asarray(_pv(ParameterDict, "p_mass_p_exports_S3"), dtype=float)
        for k in range(min(P, Q)):
    ...
    # -----------------------------
    P_to_U_raw = np.asarray(_pv(ParameterDict, "p_mass_production_to_use_S3"), dtype=float)
    if P_to_U_raw.ndim == 5:
        P_to_U_raw = P_to_U_raw[..., 0]
    if P_to_U_raw.ndim != 4:
        raise TypeError(f"p_mass_production_to_use_S3 must be 4D. Got {P_to_U_raw.shape}")

    tmp_u = np.asarray(_pv(ParameterDict, "p_mass_use_to_collection_S3"), dtype=float)
    if tmp_u.ndim == 5:
        tmp_u = tmp_u[..., 0]
    ...
    if b2 == U_expected:
        ...
        Mylog.warning("p_mass_production_to_use_S3 is (t,r,u,p). Swapping to (t,r,p,u).")
        P_to_U_trpu = np.swapaxes(P_to_U_raw, 2, 3)
    else:
        raise TypeError(f"Cannot infer u-axis for p_mass_production_to_use_S3. shape={P_to_U_raw.shape}, expected U={U_expected}")
    ...
    # -----------------------------
    U_to_i_raw = np.asarray(_pv(ParameterDict, "p_mass_use_to_collection_S3"), dtype=float)
    U_to_i_trui = U_to_i_raw[..., 0] if U_to_i_raw.ndim == 5 else U_to_i_raw
    if U_to_i_trui.ndim != 4:
        raise TypeError(f"p_mass_use_to_collection_S3 must be (t,r,u,i). Got {U_to_i_trui.shape}")
    ...
    # -----------------------------
    shape_S = (T, Rr, I, I, 1)
    S_col      = _get_S_strict("p_share_collection_routing_S3", shape_S)
    S_sort_sep = _get_S_strict("p_share_sorting_separate_S3", shape_S)
    S_sort_mix = _get_S_strict("p_share_sorting_mixed_S3", shape_S)
    S_rec_proc = _get_S_strict("p_share_recycling_routing_S3", shape_S)
    S_proc_out = _get_S_partial("p_share_rec_process_outputs_S3", shape_S)
    ...
    # -----------------------------
    if "p_mass_i_exports_S3" in ParameterDict:
        X = np.asarray(_pv(ParameterDict, "p_mass_i_exports_S3"), dtype=float)
    ...
    frac_prod_tr_i = np.asarray(_pv(ParameterDict, "p_share_recycling_fraction_to_process_S3"), dtype=float)
    if frac_prod_tr_i.ndim == 4:
        frac_prod_tr_i = frac_prod_tr_i[..., 0]  # (t,r,i)
    if frac_prod_tr_i.ndim != 3 or frac_prod_tr_i.shape[2] != I:
        raise TypeError(
            f"p_share_recycling_fraction_to_process_S3 must be (t,r,i). Got {frac_prod_tr_i.shape}, I={I}"
        )
    ...
    def _read_4yields():
        Y_name = "p_recycling_process_yields_S3"
        ...
        # Residual losses (no need for p='Verluste')
    ...
    # routing matrix: CHE -> Depoly/Pyro (must be (t,r,i,j,1))
    S_CHE_name = "p_share_CHE_routing_S3"
    ...
    # rPET_total must include CHE depoly outputs (Pfand + non-Pfand)
    ...
    out["rPET_total"] = float(out.get("rPET_from_MEC_sep", 0.0) + out.get("rPET_from_MEC_mix", 0.0) + out.get("rPET_from_CHE_mix", 0.0))
    ...
    if "p_mass_rPET_demand_bottle_S3" in ParameterDict:
        rpet_dem_bottle = float(max(_scalar(_pv(ParameterDict, "p_mass_rPET_demand_bottle_S3"), 0.0), 0.0))
    else:
        rpet_dem_bottle = 0.0
        Mylog.warning("p_mass_rPET_demand_bottle missing_S3 -> using 0.0")

    if "p_rPET_demand_otherpack_tr_S3" in ParameterDict:
        rpet_dem_other = float(max(_scalar(_pv(ParameterDict, "p_rPET_demand_otherpack_tr_S3"), 0.0), 0.0))
    else:
        rpet_dem_other = 0.0
        Mylog.warning("p_rPET_demand_otherpack_tr_S3 missing -> using 0.0")
    ...
    if "p_share_recycled_output_to_PM_S2" in ParameterDict:
        Mylog.warning(
            "Parameter 'p_share_recycled_output_to_PM_S3' is present but is NOT used for rPET->PM. "
...
def export_step7_sankey_no_new_params(
    out7,
    out_dir="outputs",
    filename="sankey_Szenario S3a1_Deutschland2.html",
    font_size=16,
    ...
    # rPET totals
    # -----------------------------
    rPET_total_calc = float(rPET_from_MEC_sep + rPET_from_MEC_mix + rPET_from_CHE_sep + rPET_from_CHE_mix)
    rPET_to_PM = min(float(rPET_to_PM_raw), float(rPET_total_calc))
    ...
    # --- VISUAL ONLY split: chemsec feed PrimPET, PrimPET total stays prim_raw ---
    ...
    totals[idx[N_RPET]] = float(rPET_total_calc)

    labels = []
    for i, name in enumerate(nodes):
        if name == N_RPET:
            labels.append(
                f"{name}\n({totals[i]:.3f} kt)\n[rPET_total={rPET_total_calc:.3f} | toPM={rPET_to_PM:.3f} | open={rPET_open:.3f}]"
            )
    ...
    fig.update_layout(
        title="PET/Polymer Verpackungen – Deutschland 2040, MFA Szenario S3a1 (kt)",
...
def export_step7_nodes_links_excel_S3(
    out7,
    out_dir="outputs",
    excel_filename="nodes_links_S3a1.xlsx",
    use_short_labels=True,
):
    import numpy as np
    import pandas as pd
    ...
    # -------------------------------------------------
    # S3 CHE internal split (optional keys)
    # - if keys exist -> add Depoly/Pyro nodes + their loss nodes
    # - else fallback to aggregated CHE nodes
    # -------------------------------------------------
    ...
    # aggregated CHE nodes always exist (they’re labels)
    nodes += [N_EV_CHE_SEP, N_LOS_CHE_SEP, N_EV_CHE_MIX, N_LOS_CHE_MIX]

    # add internal CHE nodes if present
    ...
    # optional: show chemsec -> primPET production (visual only)
    ...
    # CHE (SEP): internal split if keys exist, else aggregated
    if has_che_internal_sep:
        L(N_CHE_SEP, N_DEPOLY_SEP, CHE_depoly_in_sep)
        L(N_CHE_SEP, N_PYRO_SEP,   CHE_pyro_in_sep)
        ...
        # if EV at CHE sep exists, keep it on CHE node (not on depoly/pyro)
        L(N_CHE_SEP, N_EV_CHE_SEP, EV_CHE_sep)
    ...
    # CHE (MIX): internal split if keys exist, else aggregated
    if has_che_internal_mix:
        L(N_CHE_MIX, N_DEPOLY_MIX, CHE_depoly_in_mix)
        L(N_CHE_MIX, N_PYRO_MIX,   CHE_pyro_in_mix)
...
sankey_path = export_step7_sankey_no_new_params(
    out7_ref,
    out_dir="outputs",
    filename="sankey_Szenario_S3a1_Deutschland2.html",
    font_size=12
)
Mylog.info(f"### SANKEY SAVED TO: {sankey_path}")

excel_path_S3 = export_step7_nodes_links_excel_S3(
    out7_ref,
    out_dir="outputs",
    excel_filename="nodes_links_S3a1.xlsx",
    use_short_labels=True
)
Mylog.info(f"### S3 NODES+LINKS EXCEL SAVED TO: {excel_path_S3}")
# ============================================================
