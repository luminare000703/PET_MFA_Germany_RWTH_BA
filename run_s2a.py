...
DATA_DIR = PROJECT_DIR / "data_szenario3"
...
log_filename = LOG_DIR / "PET_MFA_Szenario_S2a.log"
...
# -------------------------------------------------
ProjectSpecs_Name_ConFile = "ODYM_Config_S2a.xlsx"
...
def sample_parameter(name, V, SD, rng):
    ...
    if name in ("p_share_use_waste_type_S2a"):
        return _sample_share(V, SD, rng)

    # ROUTING MATRICES (row-normalize last axis)
    if name in (
        "p_share_collection_routing_S2a",
        "p_share_sorting_separate_S2a",
        "p_share_sorting_mixed_S2a",
        "p_share_recycling_routing_S2a",
    ...
    if name == "p_recycling_process_yields_S2a":
        return _sample_prob01(V, SD, rng)
...
def apply_hybrid_constraints_out7_relative_to_baseline(
    ...
    # if you have other aliases, mirror them too
...
def run_step7(ParameterDict, PET_MFA_System, ModelClassification, Mylog,
                  verbose=False, run_type="single", par_suffix="S2a"):
    ...
    # -----------------------------
    M_prod = np.asarray(_pv(ParameterDict, "p_mass_production_inputs_S2a"), dtype=float)  # (t,r,p,q)
    if M_prod.ndim != 4:
        raise TypeError(f"p_mass_production_inputs_S2a must be (t,r,p,q). Got {M_prod.shape}")
    ...
    if "p_mass_p_imports_S2a" in ParameterDict:
        M_imp = np.asarray(_pv(ParameterDict, "p_mass_p_imports_S2a"), dtype=float)
        for k in range(min(P, Q)):
            M_imp_pq[:, :, k, k] = np.clip(M_imp[:, :, k], 0, None)

    if "p_mass_p_exports_S2a" in ParameterDict:
        M_exp = np.asarray(_pv(ParameterDict, "p_mass_p_exports_S2a"), dtype=float)
        for k in range(min(P, Q)):
    ...
    # -----------------------------
    P_to_U_raw = np.asarray(_pv(ParameterDict, "p_mass_production_to_use_S2a"), dtype=float)
    if P_to_U_raw.ndim == 5:
        P_to_U_raw = P_to_U_raw[..., 0]
    if P_to_U_raw.ndim != 4:
        raise TypeError(f"p_mass_production_to_use_S2a must be 4D. Got {P_to_U_raw.shape}")

    tmp_u = np.asarray(_pv(ParameterDict, "p_mass_use_to_collection_S2a"), dtype=float)
    if tmp_u.ndim == 5:
        tmp_u = tmp_u[..., 0]
    ...
    if b2 == U_expected:
        ...
        Mylog.warning("p_mass_production_to_use_S2a is (t,r,u,p). Swapping to (t,r,p,u).")
        P_to_U_trpu = np.swapaxes(P_to_U_raw, 2, 3)
    else:
        raise TypeError(f"Cannot infer u-axis for p_mass_production_to_use_S2a. shape={P_to_U_raw.shape}, expected U={U_expected}")
    ...
    # -----------------------------
    U_to_i_raw = np.asarray(_pv(ParameterDict, "p_mass_use_to_collection_S2a"), dtype=float)
    U_to_i_trui = U_to_i_raw[..., 0] if U_to_i_raw.ndim == 5 else U_to_i_raw
    if U_to_i_trui.ndim != 4:
        raise TypeError(f"p_mass_use_to_collection_S2a must be (t,r,u,i). Got {U_to_i_trui.shape}")
    ...
    # -----------------------------
    shape_S = (T, Rr, I, I, 1)
    S_col      = _get_S_strict("p_share_collection_routing_S2a", shape_S)
    S_sort_sep = _get_S_strict("p_share_sorting_separate_S2a", shape_S)
    S_sort_mix = _get_S_strict("p_share_sorting_mixed_S2a", shape_S)
    S_rec_proc = _get_S_strict("p_share_recycling_routing_S2a", shape_S)
    S_proc_out = _get_S_partial("p_share_rec_process_outputs_S2a", shape_S)
    ...
    # -----------------------------
    if "p_mass_i_exports_S2a" in ParameterDict:
        X = np.asarray(_pv(ParameterDict, "p_mass_i_exports_S2a"), dtype=float)
    ...
    frac_prod_tr_i = np.asarray(_pv(ParameterDict, "p_share_recycling_fraction_to_process_S2a"), dtype=float)
    if frac_prod_tr_i.ndim == 4:
        frac_prod_tr_i = frac_prod_tr_i[..., 0]  # (t,r,i)
    if frac_prod_tr_i.ndim != 3 or frac_prod_tr_i.shape[2] != I:
        raise TypeError(
            f"p_share_recycling_fraction_to_process_S2a must be (t,r,i). Got {frac_prod_tr_i.shape}, I={I}"
        )
    ...
    if "p_mass_rPET_demand_bottle_S2a" in ParameterDict:
        rpet_dem_bottle = float(max(_scalar(_pv(ParameterDict, "p_mass_rPET_demand_bottle_S2a"), 0.0), 0.0))
    else:
        rpet_dem_bottle = 0.0
        Mylog.warning("p_mass_rPET_demand_bottle missing_S2a -> using 0.0")

    if "p_rPET_demand_otherpack_tr_S2a" in ParameterDict:
        rpet_dem_other = float(max(_scalar(_pv(ParameterDict, "p_rPET_demand_otherpack_tr_S2a"), 0.0), 0.0))
    else:
        rpet_dem_other = 0.0
        Mylog.warning("p_rPET_demand_otherpack_tr_S2a missing -> using 0.0")
    ...
    if "p_share_recycled_output_to_PM_S2a" in ParameterDict:
        Mylog.warning(
            "Parameter 'p_share_recycled_output_to_PM_S2a' is present but is NOT used for rPET->PM. "
...
def export_step7_sankey_no_new_params(
    out7,
    out_dir="outputs",
    filename="sankey_Szenario S2a_Deutschland2.html",
    font_size=16,
    ...
    fig.update_layout(
        title="PET/Polymer Verpackungen – Deutschland 2019, MFA Szenario S2a (kt)",
...
def export_step7_nodes_links_excel_common(
    out7,
    out_dir="outputs",
    excel_filename="nodes_links.xlsx",
    use_short_labels=True,
):
    import numpy as np
    import pandas as pd
...
sankey_path = export_step7_sankey_no_new_params(
    out7_ref,
    out_dir="outputs",
    filename="sankey_Szenario_S2a_Deutschland2.html",
    font_size=12
)
Mylog.info(f"### SANKEY SAVED TO: {sankey_path}")

excel_path_s2a = export_step7_nodes_links_excel_common(
    out7_ref,
    out_dir="outputs",
    excel_filename="nodes_links_S2a.xlsx",
    use_short_labels=True
)
Mylog.info(f"### S2a NODES+LINKS EXCEL SAVED TO: {excel_path_s2a}")
